...
>```plaintext
>wdt [cmds "..."] | [cmdsep x | cmdsep off] | [disable | off] | [enable | on] | [status]
>```
...
>
>wdt cmdsep ;
>wdt cmd "stopall; sysclear; ipl 120"
>wdt enable
>```

One additional command, 'pause xx', can be included for delay between commands. The pause value is seconds to delay.
'pause' is only valid when multiple commands are defined and is not included in panel command history.

>```plaintext
>wdt cmdsep ;
>wdt cmd "ssd; pause 30; stopall; sysclear; ipl 120"
>wdt enable
>```

If pause is required, a script is probably a better solution.
...
{ execute cmds } is a temporary execution state where Hercules commands from 'wdt cmds="..."' are parsed and executed.
