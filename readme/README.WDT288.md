...
Diag 0x288 implements a LPAR Watchdog Timer which a guest OS can use to recognize some
failure requiring an LPAR (Hercules) action to restore the guest OS to an operating state.
Z/Linux is a guest OS that can use the Watchdog Timer.

Linux source ([drivers/watchdog/diag288_wdt.c](https://github.com/torvalds/linux/blob/6916d5703ddf9a38f1f6c2cc793381a24ee914c6/drivers/watchdog/diag288_wdt.c)) was used as the development reference for Hercules Watchdog Timer. Hercules implementation is closer to a Z/VM implementation as various command actions can be specified if the timer expires. The difference is Z/Vm commands are specified by the guest OS when the Watchdog timer is opened, whereas Hercules expiry commands are specified by the 'wdt' command prior to enabling the watchdog timer.
...
- enabled - active     : The quest OS has started/opened the watchdog timer and the timer is active (counting down). Guest OS pings the timer to reset the countdown time.
If the countdown time expires, the specified wdt commands will be executed.
...
>```plaintext
>wdt [cmds "..."] | [cmdsep x | cmdsep off] | [disable | off] | [enable | on] | [status] | [debug [on | off]] | [$expire] | [$start] | [$stop]
>``````
...
Multiple commands can be specified if a command line separator is defined using the 'wdt cmdsep' command.
...
The commands can not include the WDT command. 

Note: the command can not be null (wdt cmd "") if the state is 'enabled - active'.

The default is null; no commands are defined. Therefore, two wdt configuration statements
...
**cmdsep x | cmdsep off**: specify the separator between commands defined by 'wdt cmds' Off resets the separator to none.
...
>```plaintext
>HHC01959I Diagnose 0x288 Watchdog Timer: status:
...
**$expire**: If the watchdog timer state is 'enabled - active', simulate a timer expiry by setting the current timeout to zero, wait 0.25 seconds (for the timer to expire) and then reset the timer with the default timeout, 30 seconds.

**$start**:  If the watchdog timer state is 'enabled-inactive', start the watchdog timer with a 30 second timeout.

**$stope**: If the watchdog timer state is 'enabled-active', stop the watch dog timer.
...
Watchdogs timer state becomes 'enabled - active' with the activation of the wdt288_thread.
...
Watchdogs timer state becomes 'enabled - inactive' with the termination of the wdt288_thread.

### State Transitions

The watchdog timer transitions between three states from wdt commands and the guest OS issuing Diag 288 instructions, as follows:

>```plaintext
>                 +---------- 'wdt disable' ------------+
>                 |                                     |
>                 v                                     |
>(start)-> [ disabled ]  ---- 'wdt enable' --------> [ enabled-inactive ]
>                                                       |     ^
>                                                       |     |
>+---------------------+      +----- 'subcode 1' -------+     |
>|                     |      |                               |
...
{ execute cmds } is a temporary execution state where Hercules commands from 'wdt cmds="..."' are parsed and executed in an separate async thread.
...
If the Hercules watchdog timer has been enabled, log entries will show wdt288_thread started and ended.

>```plaintext
>20:32:37 HHC00100I Thread id 0000e7ae9988c160, prio -1, name 'wdt288_thread' started
>...
>...
>20:32:38 HHC00101I Thread id 0000e7ae9988c160, prio -1, name 'wdt288_thread' ended
>```
...
will expire in 30 seconds and the 'wdt cmds' commands will be processed. For testing use a comment!
...
shutdowns (eg. entering Hercules commands: ssd) and avoid log messages such as:
...
>```plaintext
>23:41:13 [  OK  ] Finished snapd.seeded.service - Wait until snapd is fully seeded.
>23:41:13 [  OK  ] Reached target multi-user.target - Multi-User System.
>23:41:13 [  OK  ] Reached target graphical.target - Graphical Interface.
>23:41:13          Starting diag288_wdt.service - For__load diag288_wdt watchdog module...
>23:41:13 HHC00100I Thread id 0000f917878ed160, prio -1, name 'wdt288_thread' started
>23:41:13 [  OK  ] Finished diag288_wdt.service - Force-load diag288_wdt watchdog module.
>23:41:14 [!p]104\
>23:41:15 [!p]104\
>23:41:16 ]3008;start=3b2d422da2b04b90b259e874b7f661a1;user=root;hostname=ububtu26-04;machineid=7595e58df90147549b6c803b632c92ea;bootid=fff2f0b7538e46098c20c58c4f5a8035;pid=1213;>pidfdid=16497;comm=(agetty);servicename=serial-getty@sclp_line0.service;invocationid=ff1761833d9f4685aa4b9f62768bd66b;type=service\P+q6E616D65\
>23:41:18
>23:41:18 Ubuntu 26.04 LTS ubuntu26-04 sclp_line0
>23:41:18
>23:41:18 ubuntu26-04 login:
>```

**5.** A simulated timer expiry can be forced with the 'wdt $expire' command on the Hercules console. The commands specified in 'wdt cmds "...."' will be executed!
