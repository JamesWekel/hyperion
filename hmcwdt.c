...
static void* hmcwdt_thread( void* arg )
{
    ...
    while ( true )
    {
        OBTAIN_HMCWDT_LOCK();
        {
            ...
            if ( sysblk.hmcwdt_expire_time < hmcwdt_get_expire_time( 0 ) && !sysblk.hmcwdt_doing_cmds )
            {
...
U64 hmcwdt_get_expire_time( U32 timeout )
{
...

    {
        ...
        {
            // "Diagnose 0x288 Watchdog Timer: %s: %s"
            WRMSG( HHC01957, "E", "clock_gettime failed", strerror(errno));
        }
        ...
        {
            // "Diagnose 0x288 Watchdog Timer: %s: %s"
            WRMSG( HHC01957, "E", "gettimeofday failed", strerror(errno));
        }
...
/*-----------------------------------------------------------*/
/*  HMCWDT Command: Get Timeout (remaining seconds)          */
/*-----------------------------------------------------------*/
...
/*-----------------------------------------------------------*/
/* HMCWDT Command: Test Timer Expiry                         */
/*-----------------------------------------------------------*/
...
/*-----------------------------------------------------------*/
/*  HMCWDT Command: Set Expiry Commands                      */
/*-----------------------------------------------------------*/
/* return: 0: OK                                             */
...
/*-----------------------------------------------------------*/
int hmcwdt_set_expiry_cmds( char* cmds )
{
    int cmdslen = 0;
...
/*-----------------------------------------------------------*/
/*  HMCWDT Command: Set Command Separator                       */
/*-----------------------------------------------------------*/
/* return: 0: OK                                             */
/*-----------------------------------------------------------*/
