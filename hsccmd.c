...
int hmcwdt_cmd( int argc, char* argv[], char* cmdline )
{
    ...
    else if (strcasecmp( argv[1], "CMDS" ) == 0)
    {
        ...
        switch ( hmcwdt_set_expiry_cmds( cmds ) )
        {
        case  0:            // ok
        ...
        case -3:            // "Diagnose 0x288 Watchdog Timer: Error in function %s: %s"
    ...
    else if ( strcasecmp( argv[1], "$STOP" ) == 0)
    {
        ...
        /* stop timer */
        if ( hmcwdt_cancel( HMCWDT_DEFAULT_TIMEOUT ) != 0)
        {
