...
/*           wdt on  | enable                                        */
/*           wdt [status]                                            */
/*           wdt debug [ on | off]          (for testing only)       */
/*           wdt $expire                    (for testing only)       */
/*           wdt $start                     (for testing only)       */
/*           wdt $stop                      (for testing only)       */
/*-------------------------------------------------------------------*/
int wdt_cmd( int argc, char* argv[], char* cmdline )
{
    ...
    else if ( strcasecmp( argv[1], "DEBUG" ) == 0)
    {
        ...
        WRMSG( HHC02204 , "I", "WDT DEBUG", sysblk.wdt288_debug ? "ON" : "OFF" );
        return 0;
    }
    ...
    else if ( strcasecmp( argv[1], "$EXPIRE" ) == 0)
    {
        ...
        {
            // "Watchdog timer: enabled-active, can not be disabled"
            WRMSG( HHC01959, "E", "timer is not enabled-active; can not expire timer" );
            return -1;
        }
        else
        {
    ...
    else if ( strcasecmp( argv[1], "$START" ) == 0)
    {
        ...
        {
            WRMSG( HHC01959, "W", "timer is enabled-active; can not be started" );
            return -1;
        }

        /* initialize timer */
        if ( wdt288_init( WDT288_DEFAULT_TIMEOUT ) != 0)
            {

            // "Watchdog timer: enabled-active, can not be disabled"
            WRMSG( HHC01959, "E", "watchdog timer failed to initialize" );
            return -1;
        }
    ...
    else if ( strcasecmp( argv[1], "$STOP" ) == 0)
    {
        ...
        if  ( !WDT288_IS_ENABLED_ACTIVE )
        {
            WRMSG( HHC01959, "W", "timer is not enabled-active; can not be stopped" );
            return -1;
        }

        /* stop timer */
        if ( wdt288_cancel( WDT288_DEFAULT_TIMEOUT ) != 0)
        {
