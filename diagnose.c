...
//              regs->psw.cc, regs->GR_L(15));                 /*debug*/
    ...
    /*---------------------------------------------------------------*/
    /* An exception is the only method used to communicate an error  */
    /* to the guest OS. A condition code, or a return code           */
    ...
    /*---------------------------------------------------------------*/
    {
        ...
        switch( subcode )
        {
        case WDT288_FUNC_INIT:         //subcode 0
            if ( sysblk.wdt288_debug )
            {
            ...
            if  (    0 ||
                    WDT288_IS_ENABLED_ACTIVE ||
                    timeout < WDT288_MIN_INTERVAL
                )
            {
        ...
        case WDT288_FUNC_CHANGE:       //subcode 1
            if ( sysblk.wdt288_debug )
                {
                    logmsg(">>>> WDT_FUNC_CHANGE: enter subcode:%d, timeout:%d\n", subcode, timeout);
                }

            /* Check: is enabled-inactive? */
            /* Check: minimum timeout */
            if (    0  ||
                    WDT288_IS_ENABLED_INACTIVE ||
                    timeout < WDT288_MIN_INTERVAL
                )
            {
                ARCH_DEP(program_interrupt)(regs, PGM_OPERATION_EXCEPTION);
            ...
            /* change / reset timer */
            if ( wdt288_change( timeout) != 0)
            {
                ARCH_DEP(program_interrupt)(regs, PGM_OPERATION_EXCEPTION);
            }
            break;

        case WDT288_FUNC_CANCEL:       //subcode 2
            if ( sysblk.wdt288_debug )
                {
                    logmsg(">>>> WDT_FUNC_CANCEL: enter subcode:%d, timeout:%d\n", subcode, timeout);
                }

            /* Check: is enabled-inactive? */
            if ( WDT288_IS_ENABLED_INACTIVE )
            {
                ARCH_DEP(program_interrupt)(regs, PGM_OPERATION_EXCEPTION);
            }

            /* change / reset timer */
            if ( wdt288_cancel( timeout) != 0)
            {
