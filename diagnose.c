...
//              regs->psw.cc, regs->GR_L(15));                 /*debug*/
    ...
    /*---------------------------------------------------------------*/
    {
        ...
        switch( subcode )
        {
        case HMCWDT_FUNC_INIT:         //subcode 0
            if ( sysblk.hmcwdt_debug )
            {
        ...
        case HMCWDT_FUNC_CHANGE:       //subcode 1
            if ( sysblk.hmcwdt_debug )
                {
            ...
            /* change / reset timer */
            if ( hmcwdt_change( timeout) != 0)
            {
                ARCH_DEP(program_interrupt)(regs, PGM_OPERATION_EXCEPTION);
            }
            break;

        case HMCWDT_FUNC_CANCEL:       //subcode 2
            if ( sysblk.hmcwdt_debug )
                {
