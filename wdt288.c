...
static void* wdt288_thread( void* arg )
{
    ...
    while ( true )
    {
        OBTAIN_WDT288_LOCK();
        {
            /* timer was closed or we are in shutdown mode?*/
            if ( sysblk.wdt288_canceled || sysblk.shutdown)
            {
                /* Watchdog timer is being canceled - exit the thread */
                break;
            }

            /* timer expired? */
            if ( sysblk.wdt288_expire_time < wdt288_get_expire_time( 0 ) )
            {
                RELEASE_WDT288_LOCK();
                {
                    /* expired - execute the configured commands */
                    char* cmdline = strdup( sysblk.wdt288_cmds );

                    WRMSG( HHC01958, "I", "Timer Expired. Executing", cmdline );

                    wdt288_do_cmds( cmdline, sysblk.wdt288_cmdsep );
                    free (cmdline);
                }
                OBTAIN_WDT288_LOCK();
            }
    ...
    /* already have lock */
    sysblk.wdt288_active = FALSE; /* Mark the timer as inactive */
    RELEASE_WDT288_LOCK();
...
/*-----------------------------------------------------------*/
/*  Timer Expired: Process Commands                          */
/*-----------------------------------------------------------*/
...
int wdt288_do_cmds( char* cmdline, char cmdsep )
{
    ...
    if (DIAG288_DEBUG)
        logmsg( ">>>>>>> Watchdog timer expired - executing configured commands: %s\n", cmdline );
